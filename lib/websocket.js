...
class Websocket {
  constructor(app) {
    ...
    this.server = new ws.Server({ server: app.server })
    this.server.on('connection', async (client, req) => {
      ...
      if (app.config.connect) {
        await app.config.connect(client)
      }
      ...
      client.on('message', async (data) => {
        ...
        if (action) {
          const result = await app.run('websocket', action, data, client)
